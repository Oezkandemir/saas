...
async function findOrCreateProduct(name, description) {
  ...
  const existingProduct = products.data.find(p => p.name === name && !p.deleted);
...
async function findOrCreatePrice(productId, amount, currency, interval, intervalCount) {
  ...
  if (existingPrice) {
    console.log(`      ✅ Found existing price: ${(amount / 100).toFixed(2)} ${currency.toUpperCase()} / ${intervalCount} ${interval} (${existingPrice.id})`);
  ...
  console.log(`      ✅ Created new price: ${(amount / 100).toFixed(2)} ${currency.toUpperCase()} / ${intervalCount} ${interval} (${price.id})`);
  return price;
}
...
async function syncStripePrices() {
  ...
  try {
    for (const config of priceConfig) {
      ...
      const product = await findOrCreateProduct(config.productName, config.productDescription);
    ...
    for (const config of priceConfig) {
      ...
      for (const [interval, priceInfo] of sortedPrices) {
        ...
        console.log(`${envVar}=${priceInfo.priceId}  # ${currency} ${amount} / ${interval}`);
      }
      console.log('');
    }
