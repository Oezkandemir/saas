...
async function validatePrices() {
  if (!process.env.STRIPE_API_KEY) {
    ...
    console.error('   Bitte setzen Sie STRIPE_API_KEY in Ihrer .env.local Datei');
    return;
  }
  ...
  console.log(`   API Key: ${process.env.STRIPE_API_KEY.substring(0, 12)}...${process.env.STRIPE_API_KEY.substring(process.env.STRIPE_API_KEY.length - 4)}\n`);

  const priceIds = [
    { name: 'Pro Monthly', envVar: 'NEXT_PUBLIC_STRIPE_PRO_MONTHLY_PLAN_ID', id: process.env.NEXT_PUBLIC_STRIPE_PRO_MONTHLY_PLAN_ID },
    { name: 'Pro Yearly', envVar: 'NEXT_PUBLIC_STRIPE_PRO_YEARLY_PLAN_ID', id: process.env.NEXT_PUBLIC_STRIPE_PRO_YEARLY_PLAN_ID },
    { name: 'Business/Enterprise Monthly', envVar: 'NEXT_PUBLIC_STRIPE_BUSINESS_MONTHLY_PLAN_ID', id: process.env.NEXT_PUBLIC_STRIPE_BUSINESS_MONTHLY_PLAN_ID },
    { name: 'Business/Enterprise Yearly', envVar: 'NEXT_PUBLIC_STRIPE_BUSINESS_YEARLY_PLAN_ID', id: process.env.NEXT_PUBLIC_STRIPE_BUSINESS_YEARLY_PLAN_ID },
  ];
  ...
  for (const { name, envVar, id } of priceIds) {
    ...
    if (!id.startsWith('price_')) {
      ...
      if (id.startsWith('prod_')) {
        ...
        console.log(`   → Gehen Sie zu Stripe Dashboard > Products > [Ihr Produkt] > Pricing`);
    ...
    try {
      ...
      if (!price.active) {
        ...
        console.log(`   → Aktivieren Sie sie in Stripe Dashboard oder verwenden Sie eine andere Price ID\n`);
        invalidCount++;
      } else {
        console.log(`✅ ${name}:`);
        console.log(`   Price ID: ${id}`);
      ...
      if (error.code === 'resource_missing') {
        ...
        console.log(`   Price ID: ${id}`);
        console.log(`   Environment Variable: ${envVar}`);
        console.log(`   → Diese Price ID existiert nicht in Ihrem Stripe ${mode}-Account`);
        console.log(`   → Mögliche Ursachen:`);
        console.log(`     - Price ID wurde gelöscht oder archiviert`);
        console.log(`     - Price ID stammt aus einem anderen Stripe Account`);
        console.log(`     - Sie verwenden ${mode}-Modus, aber die Price ID ist aus dem anderen Modus`);
        console.log(`   → Lösung:`);
        console.log(`     1. Gehen Sie zu Stripe Dashboard (${mode}-Modus) > Products`);
  ...
  if (invalidCount + missingCount > 0) {
    ...
    console.log('   - Dies zeigt alle verfügbaren Price IDs in Ihrem Stripe Account');
