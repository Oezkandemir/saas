...
export async function POST(req: Request) {
  try {
    ...
    if (data?.user) {
      ...
      try {
        const { sendSignupConfirmationEmail, sendWelcomeEmail } = await import("@/lib/email-client");
        ...
        try {
          ...
          logger.info("Confirmation email sent successfully", { userId: data.user.id });
        ...
        try {
          ...
          logger.info("Welcome email sent successfully", { userId: data.user.id });
        } catch (welcomeError) {
