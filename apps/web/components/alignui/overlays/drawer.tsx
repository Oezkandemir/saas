...
>(({ className, ...props }, ref) => (
  <DrawerPrimitive.Overlay
    ref={ref}
    className={cn('fixed inset-0 z-50 bg-bg-white-0/80 backdrop-blur-sm', className)}
    {...props}
  />
));
    ...
    <DrawerPortal>
      ...
      <DrawerPrimitive.Content
        ...
        className={cn(
          ...
          isRightSide && 'inset-y-0 right-0 h-full w-[min(400px,calc(100%-16px))] rounded-none',
          isLeftSide && 'inset-y-0 left-0 h-full w-[min(400px,calc(100%-16px))] rounded-none',
          isTopSide && 'inset-x-0 top-0 h-auto max-h-[85vh] rounded-none',
          !isRightSide && !isLeftSide && !isTopSide && 'inset-x-0 bottom-0 h-auto max-h-[85vh] rounded-none',
          className
        )}
        {...props}
      >
...
const DrawerFooter = ({
  ...
  <div
    className={cn('mt-auto flex flex-col gap-2 border-t border-stroke-soft-200 p-5', className)}
    {...props}
  />
);
