...
export function InvoiceFullPreview({
  ...
  const companyEmail =
    companyProfile?.company_email || "info@ihr-unternehmen.de";
  const companyWebsite =
    companyProfile?.company_website || "www.ihr-unternehmen.de";
  ...
  return (
    <div
      className="w-full max-w-[210mm] mx-auto bg-white text-black p-[20mm] pb-[10mm]"
    ...
    >
      ...
      <div className="mb-6 border-b-4 border-gray-900 pb-4">
        <div className="flex justify-between items-start">
          ...
          <div>
            ...
            {companyCountry && (
              <p className="text-sm text-gray-600">{companyCountry}</p>
            )}
          </div>
          ...
          <div className="text-right text-sm">
            {companyPhone && (
              <p className="text-gray-600">
                {t("template.phone")}: {companyPhone}
              </p>
            )}
            {companyEmail && (
              <p className="text-gray-600">
                {t("template.email")}: {companyEmail}
              </p>
            )}
            {companyWebsite && (
              <p className="text-gray-600">
                {t("template.website")}: {companyWebsite}
              </p>
            )}
            {companyVatId && (
              <p className="text-gray-600 mt-2">
                {t("template.vatId")}: {companyVatId}
              </p>
            )}
          </div>
        </div>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-100 border-y-2 border-gray-900">
                <th className="text-left p-3 font-bold text-sm">
                  {t("position")}
                </th>
                <th className="text-left p-3 font-bold text-sm">
                  {t("description")}
                </th>
                <th className="text-right p-3 font-bold text-sm">
                  {t("quantity")}
                </th>
                <th className="text-right p-3 font-bold text-sm">
                  {t("unitPrice")}
      ...
      {isInvoice && (
        <div className="mb-6 p-3 bg-gray-50 border-l-4 border-gray-900">
          <p className="text-xs text-gray-700">
            <span className="font-semibold">{t("paymentTerms")}</span>{" "}
            {document.due_date
              ? new Date(document.due_date).toLocaleDateString(undefined)
              : t("onReceipt")}{" "}
            | <span className="font-semibold">{t("paymentMethod")}</span>{" "}
            {t("paymentMethodValue")} | {t("paymentInstruction")}
          </p>
        </div>
      )}
      ...
      <div className="mb-6 text-sm text-gray-700">
        <p>{isInvoice ? t("invoiceClosing") : t("quoteClosing")}</p>
      ...
      <div className="mt-4 pt-3 border-t border-gray-400">
        <div className="text-xs text-gray-600 space-y-1">
          ...
          <p className="leading-relaxed">
            ...
            {contactPerson &&
              ` · ${t("template.managingDirector")}: ${contactPerson}`}
