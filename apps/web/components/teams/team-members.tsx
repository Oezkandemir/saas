...
export function TeamMembers({
  ...
  const handleInviteSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        t("teamMembers.inviteSuccess", { email: inviteForm.email }),
      );
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : t("teamMembers.inviteErrors.default"),
      );
    } finally {
  ...
  const handleRemoveMember = async () => {
    ...
    try {
      ...
      toast.success(
        t("teamMembers.removeSuccess", { name: selectedMember.name }),
      );
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : t("teamMembers.removeErrors.default"),
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleRoleChange = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : t("teamMembers.roleErrors.default"),
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleCancelInvitation = async (invitationId: string) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error || t("teamMembers.inviteCancelErrors.default"),
        );
      }
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : t("teamMembers.inviteCancelErrors.default"),
      );
    }
  };
  ...
  const handleResendInvitation = async (invitationId: string) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error || t("teamMembers.inviteResendErrors.default"),
        );
      }
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : t("teamMembers.inviteResendErrors.default"),
      );
    }
  };
                            ...
                            <DropdownMenuTrigger asChild>
                              ...
                              >
                                ...
                                <span className="sr-only">
                                  {t("teamMembers.memberTable.actions")}
                                </span>
                              </Button>
                            </DropdownMenuTrigger>
                ...
                {invitations.length === 0 ? (
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead>
                          {t("teamMembers.invitationTable.email")}
                        </TableHead>
                        <TableHead>
                          {t("teamMembers.invitationTable.role")}
                        </TableHead>
                        <TableHead>
                          {t("teamMembers.invitationTable.expires")}
                        </TableHead>
                        <TableHead className="w-12"></TableHead>
                      </TableRow>
                    </TableHeader>
                            ...
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                ...
                                >
                                  <MoreHorizontal className="size-4" />
                                  <span className="sr-only">
                                    {t("teamMembers.invitationTable.actions")}
                                  </span>
                                </Button>
                              </DropdownMenuTrigger>
                              <DropdownMenuContent align="end">
              ...
              <div className="grid gap-2">
                ...
                >
                  <SelectTrigger id="role">
                    <SelectValue
                      placeholder={t("teamMembers.inviteDialogRolePlaceholder")}
                    />
                  </SelectTrigger>
                  <SelectContent>
                    {canManageOwners && (
      ...
      >
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("teamMembers.changeRoleDialogDescription", {
                name: selectedMember?.name || "",
                role: selectedMember
                  ? t(`roles.${selectedMember.role.toLowerCase()}`)
                  : "",
              })}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            ...
            >
              <SelectTrigger id="new-role">
                <SelectValue
                  placeholder={t("teamMembers.changeRoleDialogPlaceholder")}
                />
              </SelectTrigger>
              <SelectContent>
                {canManageOwners && (
          ...
          <DialogFooter>
            ...
            >
              {isSubmitting
                ? t("teamMembers.updating")
                : t("teamMembers.update")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
