...
export default async function ProfilePage() {
  ...
  const subscriptionEndsAt = hasSubscription && userData?.stripe_current_period_end ? 
    new Date(userData.stripe_current_period_end) : null;
        ...
        <Card className="md:col-span-1">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-1">
              <p className="text-sm font-medium text-muted-foreground">{t("accountInfo")}</p>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium">
                  {user.last_sign_in_at ? formatDistance(new Date(user.last_sign_in_at), new Date(), { addSuffix: true }) : "N/A"}
                </span>
              </div>
              {subscriptionStatus && (
                <div className="flex items-center justify-between">
                  <span className="text-sm">{t("subscription")}</span>
                  <span className={`flex items-center text-sm font-medium ${subscriptionStatus === "active" ? "text-green-500" : "text-yellow-500"}`}>
              ...
              {subscriptionEndsAt && (
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-medium">
                    {formatDistance(subscriptionEndsAt, new Date(), { addSuffix: true })}
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="space-y-2">
              <p className="text-sm font-medium text-muted-foreground">{t("quickLinks")}</p>
            ...
            <TabsContent value="overview" className="mt-4 space-y-4">
              <Card>
                ...
                <CardContent className="space-y-6">
                  <div className="space-y-2">
                    <h3 className="text-lg font-semibold">{t("personalInfo.title")}</h3>
                    <div className="grid gap-4 md:grid-cols-2">
                      <div className="space-y-1">
                        <p className="text-sm font-medium text-muted-foreground">{t("personalInfo.name")}</p>
                        <p>{user.name || t("personalInfo.notProvided")}</p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-sm font-medium text-muted-foreground">{t("personalInfo.email")}</p>
                        <p>{user.email}</p>
                      </div>
                      <div className="space-y-1">
                      ...
                      <div className="space-y-1">
                        <p className="text-sm font-medium text-muted-foreground">{t("emailVerified")}</p>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              {userData.stripe_price_id ? t("premiumPlan") : t("basicPlan")}
                            </p>
                          </div>
                          <Link href="/dashboard/billing">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="rounded-lg border p-4">
                      <p className="mb-2 text-sm text-muted-foreground">{t("recentLogins")}</p>
                      <div className="space-y-2">
                        {user.last_sign_in_at && (
                          <div className="flex items-center justify-between text-sm">
                            <span>{t("lastLogin")}</span>
                            <span>{formatDistance(new Date(user.last_sign_in_at), new Date(), { addSuffix: true })}</span>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                <CardContent>
                  ...
                  {tickets.length > 0 ? (
                    ...
                    <div className="rounded-lg border p-6 text-center">
                      ...
                      <h3 className="mb-2 text-xl font-semibold">{t("noTickets")}</h3>
