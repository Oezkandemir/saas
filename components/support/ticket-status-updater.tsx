...
export function TicketStatusUpdater({ 
  ...
  const statusOptions: Array<typeof currentStatus> = ["open", "in_progress", "resolved", "closed"];
    ...
    try {
      ...
      if (!result.success) {
        toast({
          ...
          description: result.error || "An error occurred while updating the ticket status."
        });
        return;
      }
        ...
        <DropdownMenuContent align="end">
          {statusOptions.map((statusOption) => (
            ...
            >
              ...
              {statusOption === status && <CheckCircle2 className="ml-auto size-4" />}
            </DropdownMenuItem>
          ))}
        </DropdownMenuContent>
      </DropdownMenu>
