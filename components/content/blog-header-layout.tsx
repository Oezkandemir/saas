    ...
    <>
      <MaxWidthWrapper className="py-6 md:pb-8 md:pt-10">
        <div className="max-w-screen-sm">
          <h1 className="font-heading text-3xl md:text-4xl">
            {data?.title ? t(`categories.${data.slug}.title`) : t("title")}
          </h1>
          <p className="mt-3.5 text-base text-muted-foreground md:text-lg">
            {data?.description ? t(`categories.${data.slug}.description`) : t("description")}
          </p>
        </div>
      ...
      <Drawer.Root open={open} onClose={closeDrawer}>
        ...
        >
          ...
          <p className="ml-2.5 text-sm font-medium">{t("categoriesLabel")}</p>
        </Drawer.Trigger>
