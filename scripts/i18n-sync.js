...
console.log(`Found ${langFiles.length} language files: ${langFiles.join(', ')}`);
...
for (const lang of langFiles) {
  ...
  if (missingInSource.length > 0) {
    console.log(`Found ${missingInSource.length} keys in ${lang} that don't exist in ${SOURCE_LANG}`);
    ...
    console.log(`Updated source language file with ${missingInSource.length} new keys.`);
  } else {
