...
export async function getTicketWithMessages(ticketId: string): Promise<ActionResult<{ticket: Ticket, messages: TicketMessage[]}>> {
  const user = await getCurrentUser();
...
export async function addTicketMessage(ticketId: string, formData: FormData): Promise<ActionResult<TicketMessage>> {
  const user = await getCurrentUser();
