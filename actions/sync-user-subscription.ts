...
export async function syncUserSubscriptionFromStripe() {
  try {
    ...
    if (!customers.data.length) {
      return { success: false, message: "No Stripe customer found for this email" };
    }
    ...
    if (error) {
      ...
      return { success: false, message: "Failed to update subscription data in database" };
    }
    ...
    return { success: false, message: "An error occurred while syncing subscription" };
  }
} 