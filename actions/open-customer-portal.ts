...
export async function openCustomerPortal(
  ...
  try {
    ...
    console.log(`Attempting to create Stripe portal session for customer: ${userStripeId}`);
    ...
    try {
      ...
      console.log(`Successfully created Stripe portal session, URL: ${stripeSession.url}`);
      ...
      if (stripeError.code === 'resource_missing' && stripeError.message.includes('Customer')) {
        console.log(`Customer ${userStripeId} not found in Stripe, trying to find by email: ${session.user.email}`);
        ...
        if (customers.data.length > 0) {
          ...
          console.log(`Successfully created Stripe portal session with corrected ID`);
          redirect(stripeSession.url as string);
        } else {
        ...
        if (process.env.NODE_ENV !== "production") {
          console.log(`Redirecting to test portal due to error: ${stripeTestPortalUrl}`);
          redirect(stripeTestPortalUrl);
        } else {
          throw stripeError;
        }
    ...
    console.error(`Failed to generate Stripe customer portal: ${error.message}`);
    
    // In test/development, redirect to the test portal as a fallback
    if (process.env.NODE_ENV !== "production") {
      console.log(`Redirecting to test portal after error: ${stripeTestPortalUrl}`);
      redirect(stripeTestPortalUrl);
    }
