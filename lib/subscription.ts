...
export async function getUserSubscriptionPlan(
  ...
  try {
    ...
    const userResult = await supabaseAdmin
      .from('users')
      .select('id, email, stripe_customer_id, stripe_subscription_id, stripe_price_id, stripe_current_period_end')
      .eq('id', userId)
      .single();
    ...
    if ((!userData || error) && userEmail) {
      ...
      if (emailResult.data && !emailResult.error) {
        console.log(`Found user by email ${userEmail} with ID ${emailResult.data.id}`);
        ...
        if (updateResult.error) {
          console.error(`Failed to update user ID from ${userData.id} to ${userId}:`, updateResult.error);
        } else {
    ...
    if (error || !userData) {
      console.warn(`User with id ${userId} not found in Supabase database:`, error?.message || 'No data returned');
      return DEFAULT_FREE_PLAN;
    }
